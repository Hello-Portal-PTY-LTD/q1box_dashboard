...
const MapAnalysis = ({blur}) => {
  ...
  const timePeriod = useSelector((state) => state.config.analytics.timePeriod)
  ...
  const geoAnalysisDummy = data.map((item) => {
    const values = data.map((item) => item.count)
    const maxValue = Math.max(...values)
    return {
      name: item._id,
      count: (item.count / maxValue).toFixed(1),
    }
  })
  const geoUrll = {
    type: 'Topology',
  ...
  return (
    <div>
      <div className='t-bg-white t-border t-rounded-xl t-shadow-sm t-p-8 t-flex t-flex-col t-items-center lg:t-max-h-[600px] lg:t-max-w-[600px]'>
        <p className='t-text-[18.7px] t-font-medium'>Geo Analysis</p>
          ...
          <Geographies geography={geoUrll}>
            {({geographies}) => {
              return geographies.map((geo) => {
                const d = geoAnalysisDummy.find((s) => s.name === geo.properties.name)
                return (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}

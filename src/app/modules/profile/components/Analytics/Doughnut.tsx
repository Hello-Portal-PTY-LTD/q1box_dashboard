...
const DoughnutChart: React.FC<DoughnutChartProps> = ({type}) => {
  ...
  useEffect(() => {
    const chartContainerRef = chartContainer.current // Copy chartContainer.current to a variable inside the effect

    createChart()

    return () => {
      // Clean up the chart on component unmount
      if (chartContainerRef?.chart) {
        chartContainerRef.chart.destroy()
      }
    }
